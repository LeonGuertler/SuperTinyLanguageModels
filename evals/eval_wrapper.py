        ...
        for batch_requests in batch(list(zip(prefixes, continuations)), batch_size=8):
            with torch.no_grad():
                with torch.autocast(device_type=device_str):
                    ...
                    embedding_model: embedding_models.EmbedderInterface = self.model_shell.embedding_model
                    # tokenize the inputs
                    context_tokens = [
                    ...
                    unpadded_input_tokens = [
                        (context_tokens[i] + target_tokens[i])[:-1] # shifts the tokens by 1 back
                        for i in range(len(context_tokens))
                    ]
                    unpadded_input_tokens = embedding_model.truncate(unpadded_input_tokens)
