...
class TokenizerEncoder(torch.nn.Module):
    ...
    def forward(self, x, pad_token_vector, x_ids, pad_token_id):
        ...
        # Initialize lists to store chunk indices and average chunk lengths
        chunk_indices = []
        avg_chunk_len = []
        ...
        for batch in range(batch_size):
            ...
            for i in range(num_chunks):
                start = starts[i]
                end = ends[i]  # Corrected: Removed the additional +1
...
class ByteLevelEmbedder(EmbedderInterface):
    ...
    def __init__(self, model_cfg):
        ...
        # Register pad_token_vector as a buffer during initialization
        pad_token_tensor = torch.tensor([self.pad_token_id], device="cuda")
        pad_token_vector = self.byte_embedder(pad_token_tensor)
