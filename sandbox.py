...
# HF_TOKEN = "your_hf_token_here"  # 🡒 Replace this with your Hugging Face access token

# Define your repository name
REPO_NAME = "SuperTinyLanguageModels/basic-eval-prompts"  # You can change this to your desired repository name
...
try:
    repo_url = api.create_repo(
        repo_id=REPO_NAME,
        repo_type="dataset",  # Use "model" if uploading a model
        # token=HF_TOKEN,
        exist_ok=True  # Set to True to avoid error if repo already exists
    )
    print(f"Repository '{REPO_NAME}' created successfully at {repo_url}")
except Exception as e:
    print(f"An error occurred while creating the repository: {e}")
    exit(1)

# Define your GENERATION_PROMPTS data
GENERATION_PROMPTS = [
    {
      "difficulty": "easy",
...
with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix=".json") as tmp_file:
    json.dump(GENERATION_PROMPTS, tmp_file, indent=2)
    tmp_file_path = tmp_file.name
    print(f"Data saved to temporary file: {tmp_file_path}")

# Upload the JSON file to the Hugging Face repository
try:
    api.upload_file(
        path_or_fileobj=tmp_file_path,
        path_in_repo="generation_prompts.json",
        repo_id=REPO_NAME, #repo_url,  # e.g., "username/generation-prompts"
        repo_type="dataset",
        # token=HF_TOKEN,
    )
    print(f"'generation_prompts.json' has been uploaded to the repository '{REPO_NAME}'.")
except Exception as e:
    print(f"An error occurred while uploading the file: {e}")
finally:
    # Clean up the temporary file
    os.remove(tmp_file_path)
    print(f"Temporary file {tmp_file_path} has been removed.")
