...
NUM_PROCESSING_BATCHES = 1024
TokenizedDataType = np.uint16  # (can do since enc.max_token_value == 50256 is < 2**16)
...
class StandardProcessor:
    ...
    def process(self, example):
        """Given a sample, apply the tokenization"""
    ...
    def write_tokenized_data(self, tokenized, tokenized_data_folder):
        ...
        for split, dset in tokenized.items():
            ...
            arr = np.memmap(
                filename, dtype=TokenizedDataType, mode="w+", shape=(arr_len,)
            )
            idx = 0
            for batch_idx in tqdm(
                range(NUM_PROCESSING_BATCHES), desc=f"writing {filename}"
            ):
                # Batch together samples for faster write
                batch = dset.shard(
                    num_shards=NUM_PROCESSING_BATCHES, index=batch_idx, contiguous=True
                ).with_format("numpy")
...
def prepare_data(cfg):
    ...
    # (i.e. dual byte level)
    if (
        os.path.exists(tokenized_data_folder)
        and len(os.listdir(tokenized_data_folder)) != 0
    ):
