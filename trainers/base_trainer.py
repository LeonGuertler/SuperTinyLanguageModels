...
class BaseTrainer:
    ...
    def __init__(
        ...
        if (
            self.use_wandb and self.gpu_id == 0
        ):  ## ensures that only the first GPU logs to wandb
            self._setup_logging()
        if (
            cfg.trainer.training.run_profiler and self.gpu_id == 0
    ...
    def estimate_performance(self, eval_iters=None):
        ...
        for split in ["train", "val"]:
            ...
            loss[split] = avg_loss
            avg_perplexity = aggregate_value(
                perplexities.mean().item(), self.cfg.general.device
            )
            perplexity[split] = avg_perplexity

        evaluator_results = {}
        for evaluator in self.cfg.trainer["eval"]:
            evaluator_results[evaluator["evaluator"]] = train_eval(
